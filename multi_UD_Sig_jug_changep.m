function dydt = multi_UD_Sig_jug_changep(t,y,X,p,al,rate)
...
for i=1:4
e(i)=y(i+o+row*p+2*p*col+12+5*2*p+6+5*p+7*p+2*4*p);%1 batchp=4
end
%Judgement
for i=1:8
    J(i)=y(i+4+o+row*p+2*p*col+12+5*2*p+6+5*p+7*p+2*4*p);%2
end
%UD
for i=1:9
    for j=1:p
        UB(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+7*p+2*4*p+12+(i-1)*p);%9*p
    end
end
for i=1:12
    for j=1:p
        UT(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+(i-1)*p);%12*p
    end
end
for i=1:14
    for j=1:p
        DEL(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+(i-1)*p);%14*p
    end
end
for i=1:3
    for j=1:3
        Parp(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+14*p+(i-1)*3);%9
    end
end
for i=1:3
    for j=1:3
        Parn(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+14*p+9+(i-1)*3);%9
    end
end
for i=1:3
    for j=1:3
        Gammap(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+14*p+2*9+(i-1)*3);%9
    end
end
for i=1:3
    for j=1:3
        Gamman(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+14*p+3*9+(i-1)*3);%9
    end
end
for i=1:3
    for j=1:3
        UP(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+14*p+4*9+(i-1)*3);%9
    end
end
for i=1:3
    for j=1:3
        UN(i,j)=y(j+o+row*p+2*p*col+12+5*2*p+6+5*p+...
                 7*p+2*4*p+12+9*p+12*p+14*p+5*9+(i-1)*3);%9
    end
end
%pc_matrix
...
W1pdot=x(7)*zeros(2,3)+sum(J_even)*x(27)*(UP(1:2,:)+Gammap(1:2,:)-W1p);
W1ndot=x(7)*zeros(2,3)+sum(J_even)*x(27)*(UN(1:2,:)+Gamman(1:2,:)-W1n);
...
%LMS(2)，output
W2pdot=x(15)*zeros(1,3)+sum(J_even)*x(27)*(UP(3,:)+Gammap(3,:)-W2p);
W2ndot=x(15)*zeros(1,3)+sum(J_even)*x(27)*(UN(3,:)+Gamman(3,:)-W2n);
...
J2dot=GJ*VJ2;
VJ3=KKJ*[J(5);[e(3);e(3)].*[e(3);J(5)];e(3);e(3);J(6)];
J3dot=GJ*VJ3;
VJ4=KKJ*[J(7);[e(4);e(4)].*[e(4);J(7)];e(4);e(4);J(8)];
J4dot=GJ*VJ4;
%PC
PPdot=(x(23)*[Pdot(2:4,:);Pdot(6:9,:)]+...
       10*x(29)*[-P(1:3,:);ones(1,p)-P(4,:);-P(5:7,:)])';
ppdot=PPdot(:);
E=P(5:6,:);edot=(E(1,:)+E(2,:)-e)*x(23)+50*x(25)*[J1dot(1),J2dot(1),J3dot(1),J4dot(1)]...
                +10*x(29)*(-e);
...
DELdot=((LEFT+RIGHT-DEL)*x(27)*sum(J_even))';deldot=DELdot(:);
...
Parpdot=((Parpp-Parp)*sum(J_even)*x(27))';Parndot=((Parnn-Parn)*sum(J_even)*x(27))';
pardot=[Parpdot(:);Parndot(:)];
Gdot=[g12pdot;g3pdot;g12ndot;g3ndot];
O6dot=[ubdot;utdot;deldot;pardot;Gdot];
UPdot=(sum(J_even)*x(27)*(al*Parp-UP))';updot=UPdot(:);
UNdot=(sum(J_even)*x(27)*(al*Parn-UN))';undot=UNdot(:);
O7dot=[updot;undot];
