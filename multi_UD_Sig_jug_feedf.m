function dydt = multi_UD_Sig_jug_feedf(t,y,X,p)
row=3;col=size(X,2); %样本和batch size的设置
o=22;rate=1;k1=0.01;k2=0.001;k3=0.01;k4=0.001;kf=10;%初始化参数
% G=[-3,0,3,0;0,-3,0,3;-1,-1,1,1;1,0,-1,0;0,1,0,-1];
kk=[k1;k3;k2;k4];K=diag(kk,0);%初始化\gamma,K矩阵
kk1=8;kk2=1;kk3=2;kk4=0.36;%双稳定速率常数0.45(实际误差限在0.45-0.5之间)
for i=1:o
    x(i)=y(i);%o
end
%S矩阵
...
% PH2dot=GH1*Vph2;
%oscillator
xdot(1)=rate*(-x(1)*x(2)+x(1)*x(22));%assignment
...
xdot(22)=rate*(x(21)*x(22)-x(22)*x(1));
%Assignment
Sdot=(X*C-S)*x(1);
Sdot=Sdot';sdot=Sdot(:);
for l=1:p
...
W1pdot=x(7)*zeros(2,3);
W1ndot=x(7)*zeros(2,3);
...
HT=repmat(1/2,[2 p]);
Hpdot=(x(11)*(Np.*(HT-Hp)*5)+x(13)*(Np.*(Hp-Hp.^2)))';hpdot=Hpdot(:);
Hndot=(x(11)*(Nn.*(HT-Hn)*5)+x(13)*(Nn.*(Hn.^2-Hn)))';hndot=Hndot(:);
Hdot=(x(13)*(Hp+Hn-H))';hdot=Hdot(:);
O2dot=[hpdot;hndot;hdot];
%LMS(2)，output
W2pdot=x(15)*zeros(1,3);
W2ndot=x(15)*zeros(1,3);
...
%Y
YT=repmat(1/2,[1 p]);
Ypdot=(x(19)*(N3p.*(YT-Yp)*5)+x(21)*(N3p.*(Yp-Yp.^2)))';ypdot=Ypdot(:);
Yndot=(x(19)*(N3n.*(YT-Yn)*5)+x(21)*(N3n.*(Yn.^2-Yn)))';yndot=Yndot(:);
Ydot=(x(21)*(Yp+Yn-Y));ydot=Ydot(:);
...
% UPdot=((J(4)+J(2))*x(29)*(al*Parp-UP))';updot=UPdot(:);
% UNdot=((J(4)+J(2))*x(29)*(al*Parn-UN))';undot=UNdot(:);
% O7dot=[updot;undot];
